...
class ParsingSystem:
    ...
    def apply(self, c, t):
        ...
        # Check the label start symbol to determine the direction of arc
        if t.startswith("L"):
            # Add an arc from wj -> wi where i < j with label
            head = c.getStack(0)
            tail = c.getStack(1)
            label = t[t.find("(")+1:t.find(")")]
            c.removeSecondTopStack()
            c.addArc(head, tail, label)
        elif t.startswith("R"):
            # Add an arc from wi -> wj where i < j with label
            head = c.getStack(1)
            tail = c.getStack(0)
            label = t[t.find("(")+1:t.find(")")]
            c.removeTopStack()
            ...
            # Shift the buffer element to stack
            c.shift()
